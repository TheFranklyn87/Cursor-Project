 ...
 * Consolidated from backend/src/routes.js, osrm.js, scoring.js, cache.js
 */
...
// ── Cache (in-memory, per cold-start) ────────────────────────────────────────
...
function samplePoints(coords) {
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    ...
    const n = Math.max(1, Math.floor(segDist / SAMPLE_INTERVAL_M));
    for (let j = 0; j <= n; j++) {
      const t = j / n;
...
function getLightingCount(lat, lng) {
  ...
  const radiusCells = Math.ceil((LIGHT_RADIUS_M * M_TO_DEG) / gs);
...
function scoreRoute(route, night = false) {
  loadData();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!osmRoutes) {
      ...
      if (osmRoutes?.length) cacheSet(from.lat, from.lng, to.lat, to.lng, osmRoutes);
    }
