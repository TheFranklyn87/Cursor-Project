...
function App() {
    ...
    useEffect(() => {
        if (routes?.routes?.length) {
            setSelectedIndex(0);
            setIsMobileExpanded(false); // Auto-collapse search view on results
        }
    }, [routes, routeMode]);
    ...
    const handleMapClick = (lat, lng) => {
        ...
        setClickMode(null);
        setIsMobileExpanded(true); // Re-open the overlay if they clicked map-to-set
    };

    return (
        <div className={`app ${isMobileExpanded ? 'mobile-expanded' : ''}`}>
            <aside className="sidebar">
                ...
                {routes?.routes && optionIndicesSafe.length > 0 && !isMobileExpanded && (
